...
public class DrawerOverlay {
    ...
    public DrawerOverlay() {
        showStackRemainder = ModCommonConfig.INSTANCE.INTEGRATION.jade.stackRemainder.get();
        respectQuantifyKey = ModCommonConfig.INSTANCE.INTEGRATION.jade.respectQuantifyKey.get();
    }
    ...
    private void addContent(final List<Component> result, final BlockEntityDrawers tile, final IDrawerAttributes attr) {
        ...
        for (int i = 0; i < group.getDrawerCount(); i++) {
            ...
            Component name = Component.translatable("tooltip.vault_drawers.waila.empty");
            ...
            result.add(Component.translatable("tooltip.vault_drawers.waila.drawer", i + 1, name));
        }
    ...
    private void addStackLimit(List<Component> result, BlockEntityDrawers tile, IDrawerAttributes attr) {
        ...
        if (attr.isUnlimitedStorage() || tile.getDrawerAttributes().isUnlimitedVending())
            result.add(Component.translatable("tooltip.vault_drawers.waila.nolimit"));
        else {
            ...
            result.add(Component.translatable("tooltip.vault_drawers.waila.limit", limit, multiplier));
        }
    }
    ...
    private void addStatus(List<Component> result, BlockEntityDrawers tile, IDrawerAttributes attr) {
        ...
        if (attr.isItemLocked(LockAttribute.LOCK_POPULATED))
            attribs.add(Component.translatable("tooltip.vault_drawers.waila.locked"));
        if (attr.isVoid())
            attribs.add(Component.translatable("tooltip.vault_drawers.waila.void"));
        if (attr.isBalancedFill())
            attribs.add(Component.translatable("tooltip.vault_drawers.waila.balanced"));
        if (tile.getOwner() != null)
            attribs.add(Component.translatable("tooltip.vault_drawers.waila.protected"));
        if (attr.isMagnet())
            attribs.add(Component.translatable("tooltip.vault_drawers.waila.magnetic"));
        if (attr.isSuspended())
            attribs.add(Component.translatable("tooltip.vault_drawers.waila.suspended"));
